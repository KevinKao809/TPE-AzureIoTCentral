...
class azureIoTCentral(tpeHelper.iotHubHandler):
    ...
    async def start(self):
        ...
        try:
            ...
            print("Provisiong fail: " + str(e))
            self.connection_status = 'Fail on provisioning (register error)'
        ...
        if registration_result.status == "assigned":
            ...
            try:
                ...
                self.connection_status = 'Fail on connection'
                self.connection_ts = time.time()
                return
            else:
            ...
            print('Fail on connect to Azure IoT Central')
            self.connection_status = 'Fail on provision (status != assigned)'
            self.connection_ts = time.time()
            return
        return            
